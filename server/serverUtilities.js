...
var dirpath              = "",  // G:\\Dropbox\\sharevis-data\\
    dirpathAfter20141202 = "D:/pagedata after 20141202/";
...
var createfolder = function createfolder(id) {
  ...
  // Temporary saving the files after 20141202 in dirpathAfter20141202, without checking dirpathAfter20141202 folder exist or not.
	if ( fs.existsSync("D:/pagedata after 20141202/"+id+getdateformat()+"/") ){
    dirpathAfter20141202 = "D:/pagedata after 20141202/"+id+getdateformat()+"/";
	}
  else{
		dirpathAfter20141202 = "D:/pagedata after 20141202/"+id+getdateformat()+"/";
...
var createfolder_sharevis = function createfolder_sharevis(id) {
  ...
  // Temporary saving the files after 20141202 in dirpathAfter20141202, without checking dirpathAfter20141202 folder exist or not.
	...
	if ( fs.existsSync("D:/pagedata after 20141202/"+id+getdateformat()+"/"+"post/") ) {
...
var fill_zero_field = function fill_zero_field(res_posts) {
	for(var i = 0; i< res_posts.data.length; i++){
		if(!(res_posts.data[i].shares)){
			res_posts.data[i].shares = {"count":0};
		}

		if(!(res_posts.data[i].likes)){
			res_posts.data[i].likes = {"summary":{"total_count":0}};
		}

		if (!(res_posts.data[i].comments)){
			res_posts.data[i].comments = {"summary":{"total_count":0}};
		}
	}
	return res_posts;
};
...
var getpicture = function getpicture(userid, res_posts) {
	var pictureurl;
	graph.get(userid, {fields: "picture.height(140).width(140)"}, function(err, res) {
